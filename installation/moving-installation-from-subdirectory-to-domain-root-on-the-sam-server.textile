...
# Login to your Textpattern administration pages, navigate to the [[Basic Preferences]] panel, and change your 'Site URL' entry to reflect the new root install location (should simply be a matter of removing the subdirectory from the path). You will also need to update the file upload path (in Preferences). Again, it should simply be a matter of removing the subdirectory.
