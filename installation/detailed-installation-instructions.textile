...
All folders and files in the unzipped package are organised under a directory named after the current release. You will install the files and folders contained within this directory ([[{{Txp current version}} File Tree|full file tree]]); do not install the container directory itself!
