\nTextpattern stores all data in a MySQL database. Within that database, there are several tables, each containing a different collection of information.

This page shows for each table what type of information it contains, followed by a list of columns for that table and the intended usage (description) of those columns. Since the columns can be used to influence sorting in various list tags, non-developers may find their descriptions useful as well.

*Note:* The size limits shown here for the various table columns may differ slightly from the actual size limits found in the tables created by Textpattern (when a column is used in multiple tables, the smallest size is used). You should consider the limits shown here as authoritative. When in doubt, ask a developer on the forum.

This reference applies to Textpattern version 4.4.1. Older versions may have different table layouts.

= textpattern =

The 'textpattern' table contains the articles you create on the write tab.
...
<td align="left">ID
integer
Unique, auto incremented numerical ID of the article</td>
<td align="left">Posted
datetime
Publication date and time</td>
<td align="left">Expires
datetime
Expiration date and time (defaults to 0000-00-00 00:00:00, which indicates that the article never expires)</td>
</tr>
</tbody>
</table>
