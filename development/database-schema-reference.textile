...
fn1. The size limits shown here for various table columns may differ slightly from the actual size limits found in the tables created by Textpattern (when a column is used in multiple tables, the smallest size is used). You should consider the limits shown here as authoritative. When in doubt, ask a developer on the forum.
...
| Expires | datetime | Expiration date and time (defaults to 0000-00-00 00:00:00, which indicates the article never expires) |
