...
The @pluggable_ui()@ callback is a special one used throughout the core to allow plugin developers to redesign or alter the layout of the administration side of Textpattern.[3]
...
The @$step@ is usually the part of the page that contains the element you wish to change. On the **Write** panel, for example, the **Article image** control has a step called @article_image@ and the **Custom fields** area has one called @custom_fields@, and so on.
...
"Elements" here means any panel widgets and their associated labels, controls, and so forth; in other words, anything a user will see or interact with.
