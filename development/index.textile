...
In this example, we use the @register_callback@ function to add text immediately below the **Textile help** link in the **Write** panel. That is where the @extend_col_1@ step puts its markup:
...
p. If you recall from the @register_callback@ definition provided earlier, the first line is giving these arguments:

* @$func@ = @abc_add_text@
* @$event@ = @article_ui@
* @$step@ = @extend_col_1@

So @abc_add_text@ is the named callback in this case, and the next line of code is the callback function, namely @abc_add_text()@. Note there's no third argument (the empty single-quotes, @''@) for this function because there's no default content to alter.
