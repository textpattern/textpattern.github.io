...
There are many "functions used in Textpattern":http://phpcrossref.com/xref/textpattern/_functions/ that may be relevant to your plugin development aims.[1] 

In particular, the @register_callback()@ function is important for writing "callbacks":#sec7 (especially admin-side callbacks).[2] This is the full function definition:
...
| 2 | @$event@ | The Textpattern _event_. |
...
The @$event@ and @$step@ arguments are significant. They're built into Textpattern and can be combined in various ways to do different things. The "Core callbacks reference":http://docs.textpattern.io/development/core-callbacks-reference details all _events_ and _steps_ used to write every possible Textpattern callback. The "Admin-side events and steps":http://docs.textpattern.io/development/admin-side-events-and-steps references all the _events_ and _steps_ for admin-side plugins specifically.

fn1. As you learn about functions, be aware of the helper functions found in the __"/lib":https://github.com/textpattern/textpattern/tree/master/textpattern/lib__ folder. Some examples: The __"txplib_db.php":https://github.com/textpattern/textpattern/blob/master/textpattern/lib/txplib_db.php__ file for interacting with the Textpattern database; __"txplib_forms.php":https://github.com/textpattern/textpattern/blob/master/textpattern/lib/txplib_forms.php__ to help build different HTML form controls; __"txplib_html.php":https://github.com/textpattern/textpattern/blob/master/textpattern/lib/txplib_html.php__ to help build various other types of HTML elements; and __"txplib_misc.php":https://github.com/textpattern/textpattern/blob/master/textpattern/lib/txplib_misc.php__ to help build various “miscellaneous” functions.

fn2. Another function, @pluggable_ui@, can also be used for manipulating admin-side panels, but it's not _necessary_ for plugin development. See "The pluggable_ui function":http://docs.textpattern.io/development/the-pluggable-ui-function for more about that.

h3(#sec6-1). Public-side functions and arguments

There are four _events_ to be aware of for writing public-side callbacks. These two are mainly for comments and spam-related plugins:
...
Textpattern makes extensive use of callbacks. A callback is essentially an order (written as a function) that your plugin can use to execute some events (and steps). Hook into these known points to choose where and when your plugins run. The *"Core callbacks reference":http://docs.textpattern.io/development/core-callbacks-reference* provides complete details for all callback actions used in Textpattern, organised by type and by where in Textpattern they are called from.

The @register_callback()@ function -- and to a lesser degree, the @pluggable_ui()@ function -- is instrumental in writing callbacks, and especially for administration-related plugins. Let's put it all together with a few examples using these functions and arguments (events, steps, etc) to write callbacks.
...
p. From the "@register_callback@ definition":#sec6-2-1 provided earlier, we see the first line is giving these arguments:
...
In this case, let's say you wanted to remove the **Keywords** field.[2] You could hook into the @keywords@ step and return a space character:
