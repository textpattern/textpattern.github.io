...
** "[@register_callback()@]":#sec6-2
** "Public-side callback events":#sec6-3
** "Admin-side callback events":#sec6-4
** "@pluggable_ui()@ callback events":#sec6-5 (for admin-side layout modifications)
...
h3(#sec6-5). @pluggable_ui()@ callback events
...
When we talk about "elements" in these examples, we simply mean anything on a given admin panel that a user will see or interact with, such as a panel section or one or more of its contained form controls.
