...
| 3 | (default content) | The default markup; used if not modified. |
| 4 | (records) | The returned set of records from the database. |
...
The next line of code is the actual callback function, namely @abc_add_text()@. Because this particular event/step combination only outputs new content, that's what happens here.
