...
* "Callbacks":#sec6
** "Core callbacks reference":#sec6-1
** "The register callback":#sec6-2
** "Public-side callback events":#sec6-3
** "Admin-side callback events":#sec6-4
** "The pluggable UI callback events":#sec6-5 (for admin-side layout modifications)
* "Admin-side events and steps":#sec7
* "Functions, variables, classes, and constants":#sec8
...
Make use of these guidelines to ensure your plugin instructions are consistently structured and properly formatted:

* "Plugin user-help guidelines":http://docs.textpattern.io/development/plugin-user-help-guidelines

If you include screenshots in your plugin help, keep them updated with UI changes. Nothing looks worse or is more confusing than outdated screenshots that don't match the help text.
...
Textpattern makes extensive use of callbacks. Hook into these known points to choose where and when your plugins run.

h3(#sec6-1). Core callbacks reference

The ultimate callbacks reference for detailed information on all available callbacks:

* "Core callbacks reference":http://docs.textpattern.io/development/core-callback-reference

h3(#sec6-2). The register callback

The @register_callback()@ is at the core of using callbacks. The full function definition looks like this:
...
h3(#sec6-3). Public-side callback events
...
h3(#sec6-4). Admin-side callback events
...
h3(#sec6-5). The pluggable UI callback events

The @pluggable_ui()@ callback is a special one used throughout the core and allows you to redesign or alter the layout of the administration side of Textpattern by targeting any panel's widgets and/or contained controls.[3]

These callbacks have a different function signature from regular callbacks, which you should be aware of when writing your own functions. They usually have 4 parameters:

# @$event@
# @$step@
# default content (The markup that is used if you don't modify it.)
# record set (If applicable. The records returned from the database that pertain to this event. For example, on the **Write** panel this would be one row containing the columns from the currently edited article.) 
...
Th @$event@ parameter usually ends in @_ui@ (for "user interface") and begins with the event to which it refers. For example, on the **Write** panel the event is @article_ui@, and on the **Files** panel the event is @file_ui@. Straightforward.

The @$step@ parameter is usually the part of the panel that contains the widget or any of its controls you wish to change. For example, on the **Write** panel, the **Article image** control has a @$step@ called @article_image@ and the **Custom fields** widget has one called @custom_fields@, and so on.
...
h4(#sec6-5-1). Adding your own admin-side panel elements
...
h4(#sec6-5-2). Removing admin-side panel elements
...
h4(#sec6-5-3). Changing admin-side panel elements
...
h2(#sec7). Admin-side events and steps
...
h2(#sec8). Functions, variables, classes, and constants
