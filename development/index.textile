...
When plugin ideas aren't conceived quietly in the minds of individuals and built to fruition without a lot of fanfare (a path usually taken when a developer needs the functionality regardless of whether the community does or not), they often begin as a core "feature idea":http://forum.textpattern.com/viewforum.php?id=2 ("a shunned one":plugin-vs-core), or "plugin discussion":http://forum.textpattern.com/viewforum.php?id=13. Bouncing plugin ideas off the community first is a good way to gauge whether a plugin idea has merit -- the more interest and constructive feedback you get, the more likely the plugin is worth building.
...
* "Plugin template helper":plugin-template-help

**Plugin user-help guidelines:**

* "Plugin user-help guidelines":plugin-user-help-guidelines (and template)
...
This documentation focuses on the technological aspects of plugins, but you should also understand the "philosophy of core development":plugin-vs-core, as it relates to why few of the functional capabilities provided by plugins are ever integrated into core code.
...
fn1. You can see the code for any installed plugin by clicking its name in the table on the "**Plugins**":administration/admin/plugins-panel panel, or by installing and using "ied_plugin_composer":http://textpattern.org/plugins/588/ied_plugin_composer.
...
* "Public-side plugin tutorial":public-side-plugin-tutorial -- A basic tutorial to learn the ropes of the easiest kind of plugin to build.
...
Admin-side plugins often make use of the "**Extensions**":administration/extensions region. In other words, if an admin-side plugin is designed to provide a user with special preferences, search mechanisms, functional controls, and so forth, they would be made available on their own sub-panel under the **Extensions** region.

* "Admin-side plugin tutorial":admin-side-plugin-tutorial -- A basic tutorial to get acquainted with the tricker kinds of plugins to build.

h2(#sec4). Register your plugin developer prefix

All hopeful plugin developers must have a "plugin developer prefix":registered-textpattern-developer-prefixes.
...
The "Core callbacks reference":core-callbacks-reference provides complete details for all callback actions used in Textpattern, organized by public-side, admin-side, plugin, and function- and tag-based callbacks.[3]

fn3. There's also the "Admin-side events and steps":admin-side-events-and-steps listing for admin-side plugins specifically. This would correspond with the various "admin-side callbacks":core-callbacks-reference#sec2 in the __Core callbacks reference__, but it doesn't provide the explanatory details like the callbacks reference does.

h3(#sec6-1). Function: @register_callback()@

There are many "functions used in Textpattern":http://phpcrossref.com/xref/textpattern/_functions/ that may be relevant to your plugin development aims.[4] In particular, the @register_callback()@ function is important for writing "callbacks":#sec7 (especially admin-side callbacks).[5] This is the full function definition:
...
These resources are hosted at *phpcrossref.com* and require a password log in. You may "request a password":mailto:inbox@phpcrossref.com from the site owner to access the information:

* "Variables list":http://phpcrossref.com/xref/textpattern/_variables/ %(warning)password access only%
* "Classes list":http://phpcrossref.com/xref/textpattern/_classes/ %(warning)password access only%
* "Constants list":http://phpcrossref.com/xref/textpattern/_constants/ %(warning)password access only%

h2(#sec9). Full plugin tutorials

Here are two basic plugin tutorials to put it all in perspective:

* "Public-side plugin tutorial":public-side-plugin-tutorial
* "Admin-side plugin tutorial":admin-side-plugin-tutorial
...
* "Plugin user-help guidelines":plugin-user-help-guidelines
...
* "Textpattern repositories":txp-repositories

**Integration and waypoints:**

These may be useful depending on the kind of plugin you're building.

* "User role types and privileges":/administration/user-role-types-and-privileges (If your plugin will interact with user accounts.)
* "Database schema reference":database-schema-reference (If your plugin will need a database table.)
* "**Extensions**":/administration/extensions (If your admin-side plugin will provide __Publisher__ controls in it's own panel under **Extensions**.)
