...
|_. Plugin type |_. No. |_. Explanation |
| public | 0 | Will only be loaded on the public side. |
| admin + client | 1 | Will be loaded on both public _and_ the admin sides. |
| library | 2 | Will not automatically load, rather it loads when included/required by other plugins. |
| admin | 3 | Will only be loaded on the admin side and may _not_ make asynchronous calls. |
| admin + AJAX | 4 | Will only be loaded on the admin side and may make asynchronous calls. |
| admin + public + AJAX | 5 | Will be loaded on both the admin and public sides and may make asynchronous calls. |
...
The *"Core callbacks reference":http://docs.textpattern.io/development/core-callbacks-reference* provides complete details for all callback actions used in Textpattern, organised by public-side, admin-side, plugin, and function- and tag-based callbacks.[3]
...
There are many "functions used in Textpattern":http://phpcrossref.com/xref/textpattern/_functions/ that may be relevant to your plugin development aims.[4] 

In particular, the @register_callback()@ function is important for writing "callbacks":#sec7 (especially admin-side callbacks).[5] This is the full function definition:
...
*Argument #3:* In admin-side situations, the @$event@ (argument #2) is (disjunctively) divided into _steps_, with each @$step@ pinpointing a particular action or DOM location (e.g. a panel widget or one of its controls) in the admin-side. 
