...
bc(code). function abc_hello_world($atts, $thing=NULL) {
...
bc(code). <nowiki><txp:abc_hello_world /></nowiki>
...
bc(code). Hello ! Pleased to meet you.


Change your tag to read:

bc(code). <nowiki><txp:abc_hello_world name="Fred" /></nowiki>
...
bc(code). <txp:abc_hello_world name="Fred">
   <p>Welcome admin</p>
<txp:else />
...
bc(code). function abc_admin_hw_gui($event, $step) {
...
bc(code). function abc_admin_hw_list() {
   pagetop('My plugin tab');
   echo '<ul>';
