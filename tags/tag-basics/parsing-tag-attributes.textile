...
The @file-category@ form returns some markup (in this case, just a series of links to the relevant files) which is all assigned to the *value* of the "variable":../variable.textile named 'file-count'.

The upshot is that this variable can be tested with "if-variable":../if-variable.textile to see if it contains anything at all and - if it does, the file list can be displayed. Conversely, if there are no files for that language then the "else":../else.textile part of the conditional statement is displayed.
