...
The *search_result_count* tag is a _single_ tag that returns the number of articles returned by an article tag. Use "if_search":if-search to count search results or use in regular page after the "article":article tag. If you need the results' count _before_ the list of results, use the article tag in conjunction with @pgonly="1"@ (see example 3).

p(alert-block information). Note: The "if_search":if-search conditional tag is required to recognise actual search results, without them the number of articles is returned by default.
