...
Restrict to articles from specified category/categories. Note: the category names may be different to the title you typed when you created the category, as the names are sanitised for URL use. Check the [[Categories administration panel]] to ensure you are using the correct names.
...
p. See the "email":email tag for a similar solution for Symbolset's 'email' glyph.
