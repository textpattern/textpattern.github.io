...
When used in a page template, it will test the article identified by the attribute for status and act, or not, according to that status. It will not pass the @id@ attribute to the contained statement, such as "comments":comments.textile or "comments_form":comments-form.textile, they must be added as attributes to the contained tag.

This tag is mainly used in combination with "if_comments_disallowed":if-comments-disallowed.textile.
...
Other tags used: "comments":comments.textile, "else":else.textile, "comment_id":comment-id.textile.
