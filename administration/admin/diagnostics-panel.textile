...
The **Pre-flight checks** display is where diagnostic feedback messages appear. The feedback messsages come in four colors (green, blue, yellow, and red), which correspond with the type and severity of the feedback messages you might see.[1]

h3. Green (all is good) 

There is only one green message: 

<span style="color:#238c23; background-color:#dff0d8;">ll checks passed!</span> 
...
h3. Blue (time to update)

There is only one blue message: 

<span style="color:#3a87ad; background-color:#e7f6fc;">New Textpattern version n.n.n "available for download":http://textpattern.com/download.</span> (where "n.n.n" is the latest version number)

As it suggests, this message means your version of Textpattern is not current. For security reasons, you should update as soon as possible. When you do, your diagnostics will show some yellow messages needing addressed.

h3. Yellow (warnings) 

Yellow messages are not critical emergencies, but some may indicate problems that may prevent you from doing something in particular, such as upload files to your __/files__ directory. 

Yellow messages usually appear when you need to remove the __/update__ directory on your server and/or when directories need a change to their rights of access in order to function.[2] 

Possible yellow messages include: 

* <span style="color:#cb8f02; background-color:#fff6d3;">/usr/home/{username}/public_html/textpattern/setup/ still exists</span> -- This means you need to remove the "setup" folder from the web server. 
* <span style="color:#cb8f02; background-color:#fff6d3;">image directory is read-only</span> -- This means permissions on your images folder needs changed before you can add any images to it; change permissions to chmod 755 or 711.[3]
* <span style="color:#cb8f02; background-color:#fff6d3;">DNS lookup failed: {site URL}</span> -- This means the "Site URL" preference you have entered in the **"Basic":preferences/basic-prefs-panel** panel is probably incorrect somehow.
* <span style="color:#cb8f02; background-color:#fff6d3;">Site URL preference might be incorrect: {site URL}</span> -- Similar to the previous message, this is suggesting the "Site URL" as set in the preferences tab does not match the actual URL you use to visit your website.
* <span style="color:#cb8f02; background-color:#fff6d3;">Clean URL test failed.</span> -- This is not necessarily an error, and could appear for different reasons. Check the help information. It could be caused by the PHP configuration setting of @allow_url_fopen@, or some other hosting issue, or it might go away by fixing other warning messages.
