...
* cleanurl_only_apache %(information)txp_lang phrase needed here% -- The message informs you that clean URLs are not supported on any web server besides Apache. This only appears if you're _not_ using an Apache web server. The assumption is that you know what you're doing so it's not a real warning (yellow) or error (red) message. If the assumption is true, you can ignore this message.
* file_uploads_disabled %(information)txp_lang phrase needed here% -- This appears if the permissions you have set on your __files__ folder in the installation tree is read only. The assumption is that you've consciously made this choice, so you can ignore this message. If you want the message to go away, make the files folder writable.[1]
...
* old_placeholder_exists %(error)txp_lang phrase needed here% -- This likely means you have created an actual folder in your server file tree that shares a name with a __section__ you created in the **Sections** panel, or visa versa. This can break your site. You need to rename or remove one or the other. 
* missing_files %(error)txp_lang phrase needed here%
* mysql_table_errors %(error)txp_lang phrase needed here%
* %(error)txp_lang phrase needed here% -- This means paths to your plugin cache and __tmp__ locations are the same. Neither should be web accessible, but if you set them to the same location, a temp file could overwrite a plugin, and that would be bad. Create a different folder location for your plugin cache.
