...
The **Pre-flight checks** display is where diagnostic feedback messages appear. The feedback messsages come in four colors (green, blue, yellow, and red), which correspond with the type and severity of the feedback messages you might see.
...
Yellow messages are not critical emergencies, per se, and sometimes can even be cascading error messages (show up because of a different error)[1], or not be errors at all.[2] They sually appear when you need to remove the __/update__ directory, change rights of access on content directory on your server,[3] or when there's a problem of some kind with identifying your site identity or path. 
...
* <span style="color:#cb8f02; background-color:#fff6d3;">image directory is read-only</span> -- This means permissions on your images folder needs changed before you can add any images to it; change permissions to chmod 755 or 711.
