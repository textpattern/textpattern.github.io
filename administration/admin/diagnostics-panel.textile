...
Low info display begins with the following installation and server data. A few value descriptions are provided here for general understanding, but most should be obvious.
...
The installation and server data is followed by the contents of your @.htaccess@ file (from the installation directory), which for purposes here is the "@.htaccess@ file contents":https://github.com/textpattern/textpattern/blob/master/.htaccess for a new installation. The actual file content is the part between the two dotted lines:
...
| character_sets_dir: | |
| "n" Tables: | "n" being the number of Textpattern tables that exist in your installation (default is ??); the value also indicates if tables are "OK" or not |
...
| {/textpattern files, not shown here} | list of Textpattern configuration files from the /textpattern directory; indicates file path, revision number, and ??? number |
