---
layout: document
category: administration
published: true
title: "Users"
tags:
  - users 
  - roles
  - privileges
  - accounts
  - passwords
  - emails
---

h1. Users

The *Users* panel is the default panel of the **Admin** region. It's where user accounts are accessed and managed. The panel has two functional views. The default view is a table of all existing user accounts. The secondary view is a form that displays all data for an individual account (__i.e.__, user profile data).

On this page:

* "User accounts table":#sec1
** "Add new author button":#sec1-1
** "Change your password button":#sec1-2
** "Administrator controls":#sec1-3
* "User data form":#sec2

h2(#sec1). User accounts table

The user accounts table displays the following data for each user:

* **Login** (username; serves as link to user's existing account/profile data)
* **Real name**
* **Email**
* **Privileges** (in this sense, _role type_)[1]
* **Last log in** (date and time)

Associated with the user accounts table (a top of table) are two data editing buttons: **Add new author** and **Change your password**. Their respective labels suggest who can see and use them.

fn1. See "User role types and privileges":user-role-types-and-privileges/ for how roles and privileges match up.

h3(#sec1-1). Add new author button

The administrator, and any user having the role of __Publisher__[2], can see and use the *Add new author* button to create a new user account. Administrators and Publishers can also edit any user account by clicking the username in the **Login** column, which opens up the user data form.

All other users with lesser roles will see their account record only, and will not see the **Add new author** button.

fn2. A Publisher has the same rights and power as the site administrator, but can not delete or demote the administrator account. The administrator, however, can delete or demote the Publisher at any time.

h3(#sec1-2). Change your password button

All users can see and use the *Change your password* button at the top of the table, which functions in context of the logged in user. When clicked, it opens the **Change your password** form, which consists of a single field for your new password, and a checkbox option to have the new password emailed to you, if desired.

h3(#sec1-3). Administrator controls

Administrators and Publisher can see and use a selection list control under the user accounts table. The selection menu options are:

* Change privilege
* Reset password
* Delete

The control menu works in relation to the check boxes that begin each table record.

h2(#sec2). User data form

Administrators, publishers, and the account owner can open the user account data form by clicking the username in the **Login** column of the user accounts table. 

As the data form makes clear, usernames can not be edited here[3], and the last login date/time values are not displayed, but the **Real name**, **Email**, and **Privileges** data is editable. 

Only an administrator or __Publisher__ can change a user's privileges, which can be done here or using the administrator controls mentioned above.

fn3. The only way to change a username is to change the value directly in the database table using phpMyAdmin, the command-line, or by whatever other means the database administrators prefers.

