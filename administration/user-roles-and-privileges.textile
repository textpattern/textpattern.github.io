...
Finally there are (potentially) *frozen accounts*, which have the status of "None", meaning the accounts are preserved but the account holders are denied access to the administration side. This is useful for accounts that publish content (e.g., a magazine columnist or staff writer) but later leave the editorial team. Their content will remain safely on the public side of the website, under their own name.
...
The following tables show all account types versus access to the administration panels and functionality, with emphasis on the six publishing roles (from Publisher to Designer).[2] The following legend applies in each case:
...
**Table 1. Core panels access per role. (See legend above.)**
...
|\9. **Content** |
|_. "Articles":http://docs.textpattern.io/administration/articles-panel | %(success)Full% | %(success)Full% | %(success)Full% | %(success)Full% | %(success)Full% | %(information)Limited in 4.6%[3] | %(success)Full% | %(error)Blocked% |
...
|_. "Files":files-panel | %(success)Full% | %(success)Full% | %(success)Full% | %(warning)Limited% | %(warning)Limited% | %(error)Blocked% | %(information)Blocked in 4.6% | %(error)Blocked% |
...
|_. "Sections":http://docs.textpattern.io/administration/sections-panel | %(success)Full% | %(success)Full% | %(success)Full% | %(information)Blocked in 4.6% | %(error)Blocked% | %(error)Blocked% | %(success)Full% | %(error)Blocked% |
...
| |_. Administrator |_. Publisher |_. Managing Editor |_. Copy Editor |_. Staff Writer |_. Freelancer |_. Designer |_. None |
|_. ["Home":http://docs.textpattern.io/administration/home-region][4] | %(success)Full% | %(success)Full% | %(success)Full% | %(error)Blocked% | %(error)Blocked% | %(error)Blocked% | %(error)Blocked% | %(error)Blocked% |
|_. ["Extensions":http://docs.textpattern.io/administration/extensions-region][4] | %(success)Full% | %(success)Full% | %(warning)Limited% | %(error)Blocked% | %(error)Blocked% | %(error)Blocked% | %(error)Blocked% | %(error)Blocked% |

notextile. </div>

fn4. While these rights of access per role are indicated at the region level, it is possible that a given plugin my introduce it's own finer control that alters that pattern. For example, a developer may design an admin-side plugin that allows a Copy Editor or Staff Writer to have limited rights to the plugin's configuration functionality. If that plugin has a configuration panel under **Extensions**, then those two lower user roles will see the **Extensions** region and that particular plugin panel only. 
...
Like the administrator (an account type described earlier), the Publisher is all-powerful with respect to administration-side functionality, except a Publisher cannot delete the administrator's account, whereas an administrator can certainly delete the publisher's. 
...
Like the administrator, however, there should only be one Publisher, functionally speaking. Anyone who needed similar rights, but not the absolute same rights, should be a Manager Editor, and so forth. 
