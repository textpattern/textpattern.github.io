...
The "**Extensions**":http://docs.textpattern.io/administration/extensionswill region is not an out-of-the-box feature of Textpattern, but it will appear to site administrators and Publishers if any of the admin-side plugins that make use of it are installed. The following table shows a few examples of admin-side plugins that use the **Extensions** region. This is not a complete list, nor trying to be; it's for demonstration purposes only.[5] If a plugin provides an **Extensions** panel, it will be indicated in the associated plugin instructions.  

notextile. <div class="tabular-data" itemscope itemtype="http://schema.org/Table">

|_. Plugin |_. Extensions panel label |
...
fn5. Plugin developers should follow Textpattern's "editorial style guidelines":http://docs.textpattern.io/brand/editorial-style-guidelines for UI elements to ensure their plugins produce UI elements that are consistent with Textpattern's core presentation. With respect to UI titles, headings, and labels this means not using abbreviations, and using sentence-case (not capital-case or "code_case"). In the table above, for example, the labels "arc_redirect", "Plugin Composer", and "Bio config" should be "Redirects" (or "URL redirects"), "Plugin composer", and "Bio configuration", respectively. You are encouraged to let plugin developers know (by posting in the appropriate "plugin support threads":http://forum.textpattern.com/viewforum.php?id=79) when the appearance of their plugins' UI elements are not conventional.  
