...
// to add to their form, and then check for them, before returning anything from
// this callback
...
The administrator is later free to correct the comment-status from the comment-tab. Note, that the commenter will see a notification, that their comment is pending moderation for both unmoderated and spam. (The reason is not to leak information to spammers about what's happening.)
