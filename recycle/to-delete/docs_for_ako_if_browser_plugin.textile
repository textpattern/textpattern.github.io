...
bc(code). <txp:ako_ifBrowser checkfor="ie55">Only IE 5.5!</txp:ako_ifBrowser>


The following will only display in Opera 6 or Opera 8 the first time a user views the page in said browser

bc(code). <txp:ako_ifBrowser checkfor="op6,op8" cookie="1">Only Opera 6 or Opera 8</txp:ako_ifBrowser>


The following will only output the user agent string of the browser that is being used to view the site:

bc(code). <txp:ako_ifBrowserUA />


h3(sectionedit4#tags). Tags

<ul>
<li>cookie - This tag will set a cookie the first time a user views the page, and will only display the enclosed text the first time a user views the page. 1 = on, 0 = off. By default, this behavior is off (0).</li>
