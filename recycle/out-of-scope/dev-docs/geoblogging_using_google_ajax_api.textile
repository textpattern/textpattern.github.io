...
bc(code). <script type="text/javascript" src="http://www.google.com/jsapi?key=your API key"></script>
...
bc(code). var lat = parseFloat(url.lat);
...
bc(code).   map = new google.maps.Map2(document.getElementById("map"));
...
bc(code).         google.maps.Event.addListener (map, "moveend", function() {
...
bc(code). <?xml version="1.0" encoding="ISO-8859-1" ?>
<markers>
