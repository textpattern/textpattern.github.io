...
bc(code). ~/.bash_profile


<p>(create it if it doesn't exist)</p>
bc(code). export PATH="/usr/local/mysql/bin":$PATH
...
bc(code). http://localhost/~yourShortName/
...
bc(code). /etc/apache2/users/yourShortName.conf


:

bc(code). # http://localhost = http://localhost/~yourShortName
...
bc(code). $ sudo apachectl restart
...
bc(code). /etc/apache2/httpd.conf


bc(code). #Include /private/etc/apache2/extra/httpd-vhosts.conf
...
<p>:</p>
bc(code). 127.0.0.1       localhost txp.dev

</li>
<li><p>Use the following for</p>
bc(code). /etc/apache2/extra/httpd-vhosts.conf
