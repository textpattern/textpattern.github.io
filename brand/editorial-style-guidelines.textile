...
The main sections are organised alphabetically by topic, which may be different from what you expect, but it should help you scan entries faster and thus be more usable (feedback on your experience is welcome).
...
# It normalizes the effort to create copy (due to Oxford rules being a fair compromise across British and American English conventions), so one group of writers/editors doesn't have the upper hand on the other. Oxford rules may, in fact, be the most "International" form of English usage.[1]

The rest of these guidelines will specify conventions in context to topics.

fn1. And there's the fact Dean Allen, Textpattern's creator, is Canadian, and Canada uses _-ize_ verb spellings, as defined in Oxford usage, instead of _-ise_.

h2(#spell). Spellings

There are many differences between British and American English with respect to how words are spelled. Textpattern handles this decisively by using the "Oxford English Dictionary":http://www.oed.com, as described in the "Languages":#lang section of these guidelines.

The following table compares conventional patterns of spelling between North American regions, Great Britain, and the Oxford English Dictionary (*en-GB-oxendict*).[2] Use the Oxford conventions when writing Textpattern content.
...
Words in the table are representative of all possible words having those respective patterns. 

The main thing to note is there's little difference between British English and the Oxford English Dictionary, except for words ending in “-ize/-ise” and their related grammatical forms. Use "z" in all such forms (e.g., organize, organizer, organization, organizational).

Also note that "program" (not "programme") is the correct spelling when used in context of software only. Likewise you would never use "centre" where "center" was required in code (e.g, @text-align:center;@). And it would be "data modelling" (not "data modeling") according to the -ler/-ller convention. 

fn2. See "en.wikipedia.org/wiki/Oxford_spelling":https://en.wikipedia.org/wiki/Oxford_spelling.

h2(#quotes). Quotations marks

We , a closing quotation mark is most often used _inside_ of the trailing punctuation of a clause or full phrase, not outside, unless the quoted part has it's own punctuation.[1] Also, when quoting a quotation, use double quotation marks for the outer quote, and single quotation marks for the inner quote.
