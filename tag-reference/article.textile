...
When used on the front page, article's context will include articles from all sections set to display via 'Section appears on front page?' settings (see the [[Sections page]] for more information).

p(alert-block information). @<txp:article />@ is *not* the same as @<txp:article_custom />@ - you can "check out the differences of that tag":article-custom.textile if you're unsure of the differences!

h2. Attributes

Tag will accept content/behaviour and presentation attributes (*case-sensitive*):

h3. Content/behaviour attributes
...
: The form to be used for your customised search results output.
...
h2. Note on 'article list' vs. 'individual article' context
...
h3. Example 1: Basic use as single tag
...
Calls the @default@ article form, which may contain any variation of article output you want to create. The @default@ form cannot be deleted; it is the form you see on first viewing the [[Forms page]].
...
h3. Example 2: Specifying a form
...
bc. <txp:article form="article_listing" limit="5" />
...
h3. Example 3: Offsetting article display

Continuing from the previous examples:

bc. <txp:article listform="article_listing" limit="5" offset="2" />
...
h3. Example 4: Using pageby to split article output on a page
...
bc. <txp:article limit="5" pageby="10" />
<!-- google ad -->
