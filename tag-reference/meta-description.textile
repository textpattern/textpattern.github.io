...
The *meta_description* tag is a _single_ tag used in one of two ways, or both. One is to output description text as the content of a standard HTML @<meta>@ tag in the @<head>@ of a page template:
...
The tag is context aware, in that it will use article descriptions (if available) on individual article pages, and use section and category descriptions (again, if available) on listing pages. The tag can be added directly in a [[Page template]] or as part of an article [[Form template]]. Either way, you may also nest it in a given article tag.

p(alert-block information). *Note:* The corresponding description field has a 255 character limit by default, which includes spaces and commas. This is the MySQL database default. You can edit the default using "phpMyAdmin":http://www.phpmyadmin.net, for example.
