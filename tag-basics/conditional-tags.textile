...
"Container tags":self-closed-versus-container-tags.textile in Textpattern are great for making lists and you use *conditional tags* to get there.
...
Taking this block by block we see that the conditional "if-category":../tag-reference/if-category.textile determines if the visitor has clicked a category hyperlink. If they have, we are in the *list* view. If they have not, the "else":../tag-reference/else.textile portion is executed.

In this example, the "article":../tag-reference/article.textile in the _else_ portion is treated as an individual article, by virtue of it not being a category list. In more complex pages it may be part of a larger nested structure and therefore could be a different type of list (e.g., search results), but for now we'll take it at face value and assume it is an individual article.
...
The interesting part is if the visitor is viewing a list. In this case, the first thing that happens is to display the "category":../tag-reference/category.textile title inside HTML @<h2>@ tags.
