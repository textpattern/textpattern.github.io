...
bc(code). ../textpattern/theme/foo_theme/
...
bc(code). <nowiki>messagepane</nowiki>
...
<ul>
<li>bc(code). ../textpattern/lib/txplib_theme.php

</li>
<li>bc(code). ../textpattern/theme/classic/classic.php
...
You can create new admin-side themes that extend from one of the core themes. This allows you to customise smaller amounts (by extension) while reusing (inheriting) most of the CSS, JavaScript, and so forth from one of the core theme packages.
...
bc(code). theme::based_on('classic');
